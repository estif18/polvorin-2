-- =========================================================
-- INSERTAR DATOS MAESTROS - AZURE SQL DATABASE - PALLCA
-- =========================================================
...
IF EXISTS (SELECT * FROM explosivos)
    DBCC CHECKIDENT ('explosivos', RESEED, 0);
    
IF EXISTS (SELECT * FROM ingresos)
    DBCC CHECKIDENT ('ingresos', RESEED, 0);
    
IF EXISTS (SELECT * FROM salidas)
    DBCC CHECKIDENT ('salidas', RESEED, 0);
    
IF EXISTS (SELECT * FROM devoluciones)
    DBCC CHECKIDENT ('devoluciones', RESEED, 0);
    
IF EXISTS (SELECT * FROM usuarios WHERE email != 'admin@pallca.com')
    DBCC CHECKIDENT ('usuarios', RESEED, (SELECT MAX(id) FROM usuarios));
    
IF EXISTS (SELECT * FROM stock_diario)
...
PRINT CONCAT('✅ Insertados ', @@ROWCOUNT, ' tipos de explosivos');
...
PRINT CONCAT('✅ Insertados ', @@ROWCOUNT, ' usuarios del sistema');
...
PRINT '';
PRINT '📋 RESUMEN:';
PRINT CONCAT('   💥 Explosivos: ', (SELECT COUNT(*) FROM explosivos WHERE activo = 1));
PRINT CONCAT('   👥 Usuarios: ', (SELECT COUNT(*) FROM usuarios WHERE activo = 1));
PRINT '';
PRINT '⚠️  IMPORTANTE: ';
